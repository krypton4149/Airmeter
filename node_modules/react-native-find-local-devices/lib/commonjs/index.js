...
class PortScanner {
  ...
  init = () => {
    this._listeners.push(_reactNative.DeviceEventEmitter.addListener('FLD_NEW_DEVICE_FOUND', device => {
      // This listener will be activated at the moment when the device has been found.
      // FORMAT: {ipAddress: "192.168.1.66", port: 70}
      this.onDeviceFound(device);
    }));
    this._listeners.push(_reactNative.DeviceEventEmitter.addListener('FLD_RESULTS', devices => {
    ...
    this._listeners.push(_reactNative.DeviceEventEmitter.addListener('FLD_CHECK', device => {
      // This listener will be activated in that moment when package checking a device.
      // FORMAT: {ipAddress: "192.168.1.65", port: 70}
      this.onCheck(device);
    }));
    this._listeners.push(_reactNative.DeviceEventEmitter.addListener('FLD_NO_DEVICES', () => {
      // This listener will be activated at the end of discovering.
      this._listeners.forEach(l => l.remove());
      this.onNoDevices();
    }));
    this._listeners.push(_reactNative.DeviceEventEmitter.addListener('FLD_CONNECTION_ERROR', error => {
...
var _default = exports.default = PortScanner;
